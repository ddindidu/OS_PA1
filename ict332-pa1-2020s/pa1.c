...
int parse_command(char *command, int *nr_tokens, char *tokens[])
{
    ...
    *nr_tokens = 0;
	...
	while (*curr != '\0') {
        ...
        else if (isspace(*curr) && !quotation_on) {  
			*curr = '\0';
			token_started = false;
		}
        else {
...
{
	/* Jisu implemented function 'run_command'. */

	if (strncmp(tokens[0], "exit", strlen("exit")) == 0) {
		return 0;
	}
    else if(strncmp(tokens[0], "prompt", strlen("prompt"))==0 && nr_tokens==2 ){
        ...
        int i=0;
        char **innercmd = (tokens)+2;
        
        for(i=0; i<number; i++){
            run_command(nr_tokens-2, innercmd);
        }
    }
    else{
        int waitstatus;
